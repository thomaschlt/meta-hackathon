    ...
    try:
        ...
        async with session.get(story_url) as story_response:
            if story_response.status == 200:
                ...
                if comment_elements:
                    first_comment_text = comment_elements[0].get_text(separator=' ', strip=True)
                    return first_comment_text[:200] + "..." if len(first_comment_text) > 200 else first_comment_text
        return "N/A"
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for hit in data['hits']:
                ...
                date = datetime.fromtimestamp(date_timestamp).strftime('%Y-%m-%d %H:%M:%S') if date_timestamp else 'N/A'
                ...
                if 'objectID' in hit:
                    ...
                    tasks.append(asyncio.Future())  # Add a placeholder for stories without IDs
